...
SUBROUTINE get_tpert(bl_pbl_physics, sf_sfclay_physics, dx, &
  ...
  elseif( bl_pbl_physics==MYJPBLSCHEME ) then
     ...
     CASE (MYJSFCSCHEME)
        ...
        if( pblh <= 0. ) call wrf_error_fatal3("<stdin>",933,&
...
subroutine zm_conv_tend_2(itimestep, dt, p8w, fracis3d, dp3d, du3d, ed3d, eu3d, md3d, mu3d, &
  ...
  real(r8) :: dsubcld(pcols, (ime-ims+1)*(jme-jms+1)) 

  integer :: ideep(pcols, (ime-ims+1)*(jme-jms+1)) 
  integer :: jt(pcols, (ime-ims+1)*(jme-jms+1))    
  integer :: maxg(pcols, (ime-ims+1)*(jme-jms+1))  
                               
  integer :: lengath((ime-ims+1)*(jme-jms+1))    
...
msg )
  ...
  do jw = jts,jte
     do iw = its,ite
        lchnk = (jw-jts)*(ite-its+1) + (iw-its+1) 
        ...
        do kw = kts,kte
           ...
           state_q(1,kflip,1)   = moist(iw,kw,jw,P_QV)*multFrc      
           state_q(1,kflip,ixcldliq) = moist(iw,kw,jw,P_QC)*multFrc 
           state_q(1,kflip,ixcldice) = moist(iw,kw,jw,P_QI)*multFrc 
           ...
           delta_p                 = p8w(iw,kw,jw) - p8w(iw,kw+1,jw) 
