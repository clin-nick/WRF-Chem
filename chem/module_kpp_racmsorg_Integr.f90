...
TimeLoop: DO WHILE ( (Direction > 0).AND.((T-Tend)+Roundoff <= ZERO) &
   ...
   IF( T == Tstart ) THEN
     CALL racmsorg_IRRFun( Y, FIX, RCONST, IRR_WRK )
   ENDIF
...
SUBROUTINE racmsorg_IRRFun ( V, F, RCT, IRR )
...
END SUBROUTINE racmsorg_IRRFun
...
10      Eps = Eps*2
   ...
   SUBROUTINE decomp_racmsorg( JVS, IER )
  ...
  JVS( 1052) = W( 73 )
   ...
   END SUBROUTINE decomp_racmsorg
