...
MODULE racm_esrlsorg_aqchem_Integrator

 USE racm_esrlsorg_aqchem_Parameters
 USE racm_esrlsorg_aqchem_Precision
 USE racm_esrlsorg_aqchem_JacobianSP
...
SUBROUTINE  racm_esrlsorg_aqchem_INTEGRATE( TIN, TOUT, &
  ...
  ICNTRL_U, RCNTRL_U, ISTATUS_U, RSTATUS_U, IERR_U  )
   ...
   CALL racm_esrlsorg_aqchem_Rosenbrock(VAR, FIX, RCONST, TIN,TOUT,   &
         ATOL,RTOL,               &
...
END SUBROUTINE  racm_esrlsorg_aqchem_INTEGRATE


SUBROUTINE  racm_esrlsorg_aqchem_Rosenbrock(Y, FIX, RCONST, Tstart,Tend, &
           AbsTol,RelTol,            &
           RCNTRL,ICNTRL,RSTATUS,ISTATUS,IRR_WRK,IERR)
  ...
  INTEGER :: Nfun,Njac,Nstp,Nacc,Nrej,Ndec,Nsol,Nsng
   ...
   ELSE
      ...
      CALL racm_esrlsorg_aqchem_ros_ErrorMsg(-2,Tstart,ZERO,IERR)
      RETURN
   END IF
   ...
   ELSE
      ...
      CALL racm_esrlsorg_aqchem_ros_ErrorMsg(-1,Tstart,ZERO,IERR)
      RETURN
   END IF


   Roundoff = racm_esrlsorg_aqchem_WLAMCH('E')
   ...
   ELSE
      ...
      CALL racm_esrlsorg_aqchem_ros_ErrorMsg(-3,Tstart,ZERO,IERR)
      RETURN
   END IF
   ...
   ELSE
      ...
      CALL racm_esrlsorg_aqchem_ros_ErrorMsg(-4,Tstart,ZERO,IERR)
      RETURN
   END IF
   ...
   END IF
    ...
    DO i=1,UplimTol
      IF ( (AbsTol(i) <= ZERO) .OR. (RelTol(i) <= 10.0_dp*Roundoff) &
        ...
        CALL racm_esrlsorg_aqchem_ros_ErrorMsg(-5,Tstart,ZERO,IERR)
        RETURN
      END IF
    END DO
   ...
   SELECT CASE (Method)
     CASE (1)
       CALL racm_esrlsorg_aqchem_Ros2(ros_S, ros_A, ros_C, ros_M, ros_E,   &
          ros_Alpha, ros_Gamma, ros_NewF, ros_ELO, ros_Name)
     CASE (2)
       CALL racm_esrlsorg_aqchem_Ros3(ros_S, ros_A, ros_C, ros_M, ros_E,   &
          ros_Alpha, ros_Gamma, ros_NewF, ros_ELO, ros_Name)
     CASE (3)
       CALL racm_esrlsorg_aqchem_Ros4(ros_S, ros_A, ros_C, ros_M, ros_E,   &
          ros_Alpha, ros_Gamma, ros_NewF, ros_ELO, ros_Name)
     CASE (4)
       CALL racm_esrlsorg_aqchem_Rodas3(ros_S, ros_A, ros_C, ros_M, ros_E, &
          ros_Alpha, ros_Gamma, ros_NewF, ros_ELO, ros_Name)
     CASE (5)
       CALL racm_esrlsorg_aqchem_Rodas4(ros_S, ros_A, ros_C, ros_M, ros_E, &
          ros_Alpha, ros_Gamma, ros_NewF, ros_ELO, ros_Name)
     CASE DEFAULT
   ...
   CALL racm_esrlsorg_aqchem_ros_Integrator(Y,Tstart,Tend,Texit,      &
        AbsTol, RelTol,                          &
...
CONTAINS 
 ...
 SUBROUTINE  racm_esrlsorg_aqchem_ros_ErrorMsg(Code,T,H,IERR)
 ...
 END SUBROUTINE  racm_esrlsorg_aqchem_ros_ErrorMsg


 SUBROUTINE  racm_esrlsorg_aqchem_ros_Integrator (Y, Tstart, Tend, T,     &
        AbsTol, RelTol,                          &
...
TimeLoop: DO WHILE ( (Direction > 0).AND.((T-Tend)+Roundoff <= ZERO) &
   ...
   IF ( Nstp > Max_no_steps ) THEN  
      CALL racm_esrlsorg_aqchem_ros_ErrorMsg(-6,T,H,IERR)
      RETURN
   END IF
   IF ( ((T+0.1_dp*H) == T).OR.(H <= Roundoff) ) THEN  
      CALL racm_esrlsorg_aqchem_ros_ErrorMsg(-7,T,H,IERR)
      RETURN
   END IF
   ...
   CALL racm_esrlsorg_aqchem_FunTemplate(T,Y,Fcn0, RCONST, FIX, Nfun)
   IF( T == Tstart ) THEN
     CALL racm_esrlsorg_aqchem_IRRFun( Y, FIX, RCONST, IRR_WRK )
   ENDIF


   IF (.NOT.Autonomous) THEN
      CALL racm_esrlsorg_aqchem_ros_FunTimeDeriv ( T, Roundoff, Y, &
                Fcn0, dFdT, RCONST, FIX, Nfun )
   END IF


   CALL racm_esrlsorg_aqchem_JacTemplate(T,Y,Jac0, FIX, Njac, RCONST)


UntilAccepted: DO

   CALL racm_esrlsorg_aqchem_ros_PrepareMatrix(H,Direction,ros_Gamma(1), &
          Jac0,Ghimj,Pivot,Singular, Ndec,  Nsng )
   IF (Singular) THEN 
       CALL racm_esrlsorg_aqchem_ros_ErrorMsg(-8,T,H,IERR)
       RETURN
   END IF
...
Stage: DO istage = 1, ros_S
       ...
       IF ( istage == 1 ) THEN
         CALL racm_esrlsorg_aqchem_WCOPY(NVAR,Fcn0,1,Fcn,1)
      
       ELSEIF ( ros_NewF(istage) ) THEN
         CALL racm_esrlsorg_aqchem_WCOPY(NVAR,Y,1,Ynew,1)
         DO j = 1, istage-1
           CALL racm_esrlsorg_aqchem_WAXPY(NVAR,ros_A((istage-1)*(istage-2)/2+j), &
            K(NVAR*(j-1)+1),1,Ynew,1)
         END DO
         Tau = T + ros_Alpha(istage)*Direction*H
         CALL racm_esrlsorg_aqchem_FunTemplate(Tau,Ynew,Fcn, RCONST, FIX, Nfun)
       END IF 
       CALL racm_esrlsorg_aqchem_WCOPY(NVAR,Fcn,1,K(ioffset+1),1)
       DO j = 1, istage-1
         HC = ros_C((istage-1)*(istage-2)/2+j)/(Direction*H)
         CALL racm_esrlsorg_aqchem_WAXPY(NVAR,HC,K(NVAR*(j-1)+1),1,K(ioffset+1),1)
       END DO
       IF ((.NOT. Autonomous).AND.(ros_Gamma(istage).NE.ZERO)) THEN
         HG = Direction*H*ros_Gamma(istage)
         CALL racm_esrlsorg_aqchem_WAXPY(NVAR,HG,dFdT,1,K(ioffset+1),1)
       END IF
       CALL racm_esrlsorg_aqchem_ros_Solve(Ghimj, Pivot, K(ioffset+1), Nsol)
   ...
   CALL racm_esrlsorg_aqchem_WCOPY(NVAR,Y,1,Ynew,1)
   DO j=1,ros_S
         CALL racm_esrlsorg_aqchem_WAXPY(NVAR,ros_M(j),K(NVAR*(j-1)+1),1,Ynew,1)
   END DO


   CALL racm_esrlsorg_aqchem_WSCAL(NVAR,ZERO,Yerr,1)
   DO j=1,ros_S
        CALL racm_esrlsorg_aqchem_WAXPY(NVAR,ros_E(j),K(NVAR*(j-1)+1),1,Yerr,1)
   END DO
   Err = racm_esrlsorg_aqchem_ros_ErrorNorm ( Y, Ynew, Yerr, AbsTol, RelTol, VectorTol )
   ...
   IF ( (Err <= ONE).OR.(H <= Hmin) ) THEN  
      Nacc = Nacc+1
      CALL racm_esrlsorg_aqchem_WCOPY(NVAR,Ynew,1,Y,1)
      T = T + Direction*H
  ...
  END SUBROUTINE  racm_esrlsorg_aqchem_ros_Integrator



  REAL(kind=dp) FUNCTION  racm_esrlsorg_aqchem_ros_ErrorNorm ( Y, Ynew, Yerr, &
   ...
   Err  = SQRT(Err/NVAR)
    ...
    racm_esrlsorg_aqchem_ros_ErrorNorm = Err

  END FUNCTION  racm_esrlsorg_aqchem_ros_ErrorNorm



  SUBROUTINE racm_esrlsorg_aqchem_ros_FunTimeDeriv ( T, Roundoff, Y, &
                Fcn0, dFdT, RCONST, FIX, Nfun )
   ...
   CALL racm_esrlsorg_aqchem_FunTemplate(T+Delta,Y,dFdT, RCONST, FIX, Nfun)
   CALL racm_esrlsorg_aqchem_WAXPY(NVAR,(-ONE),Fcn0,1,dFdT,1)
   CALL racm_esrlsorg_aqchem_WSCAL(NVAR,(ONE/Delta),dFdT,1)

  END SUBROUTINE  racm_esrlsorg_aqchem_ros_FunTimeDeriv



  SUBROUTINE  racm_esrlsorg_aqchem_ros_PrepareMatrix ( H, Direction, gam, &
             Jac0, Ghimj, Pivot, Singular, Ndec,  Nsng  )
   ...
   DO WHILE (Singular)
     ...
     CALL racm_esrlsorg_aqchem_WCOPY(LU_NONZERO,Jac0,1,Ghimj,1)
     CALL racm_esrlsorg_aqchem_WSCAL(LU_NONZERO,(-ONE),Ghimj,1)
     ghinv = ONE/(Direction*H*gam)
     DO i=1,NVAR
       Ghimj(LU_DIAG(i)) = Ghimj(LU_DIAG(i))+ghinv
     END DO

     CALL racm_esrlsorg_aqchem_ros_Decomp( Ghimj, Pivot, ising, Ndec )
     IF (ising == 0) THEN
  ...
  END SUBROUTINE  racm_esrlsorg_aqchem_ros_PrepareMatrix



  SUBROUTINE  racm_esrlsorg_aqchem_ros_Decomp( A, Pivot, ising, Ndec )
...
CALL decomp_racm_esrlsorg_aqchem ( A, ising )
   Pivot(1) = 1
   Ndec = Ndec + 1

  END SUBROUTINE  racm_esrlsorg_aqchem_ros_Decomp



  SUBROUTINE  racm_esrlsorg_aqchem_ros_Solve( A, Pivot, b, Nsol )
   ...
   CALL racm_esrlsorg_aqchem_KppSolve( A, b )

   Nsol = Nsol+1

  END SUBROUTINE  racm_esrlsorg_aqchem_ros_Solve




  SUBROUTINE  racm_esrlsorg_aqchem_Ros2 (ros_S,ros_A,ros_C,ros_M,ros_E,ros_Alpha,&
 ...
 END SUBROUTINE  racm_esrlsorg_aqchem_Ros2



  SUBROUTINE  racm_esrlsorg_aqchem_Ros3 (ros_S,ros_A,ros_C,ros_M,ros_E,ros_Alpha,&
           ros_Gamma,ros_NewF,ros_ELO,ros_Name)
  ...
  END SUBROUTINE  racm_esrlsorg_aqchem_Ros3





  SUBROUTINE  racm_esrlsorg_aqchem_Ros4 (ros_S,ros_A,ros_C,ros_M,ros_E,ros_Alpha,&
           ros_Gamma,ros_NewF,ros_ELO,ros_Name)
  ...
  END SUBROUTINE  racm_esrlsorg_aqchem_Ros4


  SUBROUTINE  racm_esrlsorg_aqchem_Rodas3 (ros_S,ros_A,ros_C,ros_M,ros_E,ros_Alpha,&
            ros_Gamma,ros_NewF,ros_ELO,ros_Name)
  ...
  END SUBROUTINE  racm_esrlsorg_aqchem_Rodas3


  SUBROUTINE  racm_esrlsorg_aqchem_Rodas4 (ros_S,ros_A,ros_C,ros_M,ros_E,ros_Alpha,&
             ros_Gamma,ros_NewF,ros_ELO,ros_Name)
  ...
  END SUBROUTINE  racm_esrlsorg_aqchem_Rodas4




END SUBROUTINE  racm_esrlsorg_aqchem_Rosenbrock




SUBROUTINE  racm_esrlsorg_aqchem_FunTemplate( T, Y, Ydot, RCONST, FIX, Nfun )
   ...
   CALL racm_esrlsorg_aqchem_Fun( Y, FIX, RCONST, Ydot )


   Nfun = Nfun+1

END SUBROUTINE  racm_esrlsorg_aqchem_FunTemplate



SUBROUTINE  racm_esrlsorg_aqchem_JacTemplate( T, Y, Jcb, FIX, Njac, RCONST )
 ...
 USE racm_esrlsorg_aqchem_Jacobian
    ...
    CALL racm_esrlsorg_aqchem_Jac_SP( Y, FIX, RCONST, Jcb )


    Njac = Njac+1

END SUBROUTINE  racm_esrlsorg_aqchem_JacTemplate
...
SUBROUTINE racm_esrlsorg_aqchem_Fun ( V, F, RCT, Vdot )
...
END SUBROUTINE racm_esrlsorg_aqchem_Fun
...
SUBROUTINE racm_esrlsorg_aqchem_IRRFun ( V, F, RCT, IRR )
...
END SUBROUTINE racm_esrlsorg_aqchem_IRRFun
...
SUBROUTINE racm_esrlsorg_aqchem_Jac_SP ( V, F, RCT, JVS )
...
END SUBROUTINE racm_esrlsorg_aqchem_Jac_SP
...
SUBROUTINE racm_esrlsorg_aqchem_KppDecomp( JVS, IER )
...
END SUBROUTINE racm_esrlsorg_aqchem_KppDecomp



SUBROUTINE racm_esrlsorg_aqchem_KppDecompCmplx( JVS, IER )
...
END SUBROUTINE racm_esrlsorg_aqchem_KppDecompCmplx


SUBROUTINE racm_esrlsorg_aqchem_KppSolveIndirect( JVS, X )
...
END SUBROUTINE racm_esrlsorg_aqchem_KppSolveIndirect


SUBROUTINE racm_esrlsorg_aqchem_KppSolveCmplx( JVS, X )
...
END SUBROUTINE racm_esrlsorg_aqchem_KppSolveCmplx
...
SUBROUTINE racm_esrlsorg_aqchem_KppSolve ( JVS, X )
...
END SUBROUTINE racm_esrlsorg_aqchem_KppSolve
      ...
      SUBROUTINE racm_esrlsorg_aqchem_WCOPY(N,X,incX,Y,incY)
      ...
      END SUBROUTINE racm_esrlsorg_aqchem_WCOPY



      SUBROUTINE racm_esrlsorg_aqchem_WAXPY(N,Alpha,X,incX,Y,incY)
      ...
      END SUBROUTINE racm_esrlsorg_aqchem_WAXPY




      SUBROUTINE racm_esrlsorg_aqchem_WSCAL(N,Alpha,X,incX)
      ...
      END SUBROUTINE racm_esrlsorg_aqchem_WSCAL


      REAL(kind=dp) FUNCTION racm_esrlsorg_aqchem_WLAMCH( C )
      ...
      IF (First) THEN
        ...
        DO i = 17, 80
          Eps = Eps*HALF
          CALL racm_esrlsorg_aqchem_WLAMCH_ADD(ONE,Eps,Sum)
...
10      Eps = Eps*2
      ...
      racm_esrlsorg_aqchem_WLAMCH = Eps

      END FUNCTION racm_esrlsorg_aqchem_WLAMCH
     
      SUBROUTINE racm_esrlsorg_aqchem_WLAMCH_ADD( A, B, Sum )

      
      REAL(kind=dp) A, B, Sum
      Sum = A + B

      END SUBROUTINE racm_esrlsorg_aqchem_WLAMCH_ADD




      SUBROUTINE racm_esrlsorg_aqchem_SET2ZERO(N,Y)
      ...
      END SUBROUTINE racm_esrlsorg_aqchem_SET2ZERO



      REAL(kind=dp) FUNCTION racm_esrlsorg_aqchem_WDOT (N, DX, incX, DY, incY) 
      ...
      racm_esrlsorg_aqchem_WDOT = 0.0D0 
      IF (N .LE. 0) RETURN 
    ...
    5 IX = 1 
      ...
      DO i = 1,N 
        racm_esrlsorg_aqchem_WDOT = racm_esrlsorg_aqchem_WDOT + DX(IX)*DY(IY) 
        IX = IX + incX 
        IY = IY + incY 
      END DO 
      RETURN 
   ...
   20 M = MOD(N,5) 
      IF (M .EQ. 0) GO TO 40 
      DO i = 1,M 
         racm_esrlsorg_aqchem_WDOT = racm_esrlsorg_aqchem_WDOT + DX(i)*DY(i) 
      END DO 
      IF (N .LT. 5) RETURN 
   40 MP1 = M + 1 
      DO i = MP1,N,5 
          racm_esrlsorg_aqchem_WDOT = racm_esrlsorg_aqchem_WDOT + DX(i)*DY(i) + DX(i+1)*DY(i+1) +&
                   DX(i+2)*DY(i+2) +  &
   ...
   60 NS = N*incX 
      ...
      END FUNCTION racm_esrlsorg_aqchem_WDOT                                          




   SUBROUTINE decomp_racm_esrlsorg_aqchem( JVS, IER )
  ...
  JVS( 1086) = W( 80 )
   ...
   END SUBROUTINE decomp_racm_esrlsorg_aqchem
