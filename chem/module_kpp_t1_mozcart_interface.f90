...
MODULE module_kpp_t1_mozcart_interf 
  ...
  USE t1_mozcart_UpdateRconstWRF
  USE t1_mozcart_Integrator
...
SUBROUTINE  t1_mozcart_interface( &
  ...
  REAL(kind=dp) :: aer_so4, aer_oc2, aer_bc2
   ...
   RCNTRL(3) = 0.01_dp * TIME_END
    ...
    FIX(indf_O2)  = .209_dp*REAL(dens2con_a * rho_phy(i,k,j), KIND=dp)
      
    FIX(indf_N2)  = .79_dp*REAL(dens2con_a * rho_phy(i,k,j), KIND=dp)
    ...
    var(ind_ALKNIT) = conv  * REAL( MAX(chem(i,k,j,P_alknit),0.), KIND=dp)  
    ...
    var(ind_BZALD) = conv  * REAL( MAX(chem(i,k,j,P_bzald),0.), KIND=dp)  
    ...
    var(ind_CH3CN) = conv  * REAL( MAX(chem(i,k,j,P_ch3cn),0.), KIND=dp)  
    ...
    var(ind_EOOH) = conv  * REAL( MAX(chem(i,k,j,P_eooh),0.), KIND=dp)  
    ...
    var(ind_HCN) = conv  * REAL( MAX(chem(i,k,j,P_hcn),0.), KIND=dp)  
    ...
    var(ind_HONITR) = conv  * REAL( MAX(chem(i,k,j,P_honitr),0.), KIND=dp)  
    var(ind_HPALD) = conv  * REAL( MAX(chem(i,k,j,P_hpald),0.), KIND=dp)  
    var(ind_HYAC) = conv  * REAL( MAX(chem(i,k,j,P_ACETOL),0.), KIND=dp)  
    var(ind_HYDRALD) = conv  * REAL( MAX(chem(i,k,j,P_hydrald),0.), KIND=dp)  
    var(ind_IEPOX) = conv  * REAL( MAX(chem(i,k,j,P_iepox),0.), KIND=dp)  
    var(ind_ISOP) = conv  * REAL( MAX(chem(i,k,j,P_ISOPR),0.), KIND=dp)  
    var(ind_ISOPNITA) = conv  * REAL( MAX(chem(i,k,j,P_isopnita),0.), KIND=dp)  
    var(ind_ISOPNITB) = conv  * REAL( MAX(chem(i,k,j,P_isopnitb),0.), KIND=dp)  
    var(ind_ISOPNO3) = conv  * REAL( MAX(chem(i,k,j,P_ISOPN),0.), KIND=dp)  
    var(ind_ISOPNOOH) = conv  * REAL( MAX(chem(i,k,j,P_isopnooh),0.), KIND=dp)  
    ...
    var(ind_NC4CH2OH) = conv  * REAL( MAX(chem(i,k,j,P_nc4ch2oh),0.), KIND=dp)  
    var(ind_NC4CHO) = conv  * REAL( MAX(chem(i,k,j,P_nc4cho),0.), KIND=dp)  
    ...
    var(ind_NTERPOOH) = conv  * REAL( MAX(chem(i,k,j,P_nterpooh),0.), KIND=dp)  
    ...
    var(ind_PHENOL) = conv  * REAL( MAX(chem(i,k,j,P_phenol),0.), KIND=dp)  
    ...
    var(ind_TERPNIT) = conv  * REAL( MAX(chem(i,k,j,P_terpnit),0.), KIND=dp)  
    ...
    var(ind_XYLENES) = conv  * REAL( MAX(chem(i,k,j,P_xylenes),0.), KIND=dp)  
    ...
    var(ind_ISOPAO2) = conv  * REAL( MAX(chem(i,k,j,P_isopao2),0.), KIND=dp)  
    var(ind_ISOPBO2) = conv  * REAL( MAX(chem(i,k,j,P_isopbo2),0.), KIND=dp)  
  ...
  if( p_bc2 > param_first_scalar ) then
   ...
   CALL t1_mozcart_Update_Rconst(  &


   rh, aer_so4, aer_oc2, aer_bc2, &
   config_flags%aero_srf_area_diag, &
   aero_srf_area(i,k,j,P_sulf_srf_area), &
   aero_srf_area(i,k,j,P_oc_srf_area), &
   aero_srf_area(i,k,j,P_bc_srf_area), &
  ...
  CALL t1_mozcart_INTEGRATE(TIME_START, TIME_END, &  
   ...
   ENDIF
    ...
    chem(i,k,j,P_alknit) = MAX ( REAL (oconv * var(ind_ALKNIT), KIND=sp), 0.)  
    ...
    chem(i,k,j,P_bzald) = MAX ( REAL (oconv * var(ind_BZALD), KIND=sp), 0.)  
    ...
    chem(i,k,j,P_ch3cn) = MAX ( REAL (oconv * var(ind_CH3CN), KIND=sp), 0.)  
    ...
    chem(i,k,j,P_eooh) = MAX ( REAL (oconv * var(ind_EOOH), KIND=sp), 0.)  
    ...
    chem(i,k,j,P_hcn) = MAX ( REAL (oconv * var(ind_HCN), KIND=sp), 0.)  
    ...
    chem(i,k,j,P_honitr) = MAX ( REAL (oconv * var(ind_HONITR), KIND=sp), 0.)  
    chem(i,k,j,P_hpald) = MAX ( REAL (oconv * var(ind_HPALD), KIND=sp), 0.)  
    chem(i,k,j,P_ACETOL) = MAX ( REAL (oconv * var(ind_HYAC), KIND=sp), 0.)  
    chem(i,k,j,P_hydrald) = MAX ( REAL (oconv * var(ind_HYDRALD), KIND=sp), 0.)  
    chem(i,k,j,P_iepox) = MAX ( REAL (oconv * var(ind_IEPOX), KIND=sp), 0.)  
    chem(i,k,j,P_ISOPR) = MAX ( REAL (oconv * var(ind_ISOP), KIND=sp), 0.)  
    chem(i,k,j,P_isopnita) = MAX ( REAL (oconv * var(ind_ISOPNITA), KIND=sp), 0.)  
    chem(i,k,j,P_isopnitb) = MAX ( REAL (oconv * var(ind_ISOPNITB), KIND=sp), 0.)  
    chem(i,k,j,P_ISOPN) = MAX ( REAL (oconv * var(ind_ISOPNO3), KIND=sp), 0.)  
    chem(i,k,j,P_isopnooh) = MAX ( REAL (oconv * var(ind_ISOPNOOH), KIND=sp), 0.)  
    ...
    chem(i,k,j,P_nc4ch2oh) = MAX ( REAL (oconv * var(ind_NC4CH2OH), KIND=sp), 0.)  
    chem(i,k,j,P_nc4cho) = MAX ( REAL (oconv * var(ind_NC4CHO), KIND=sp), 0.)  
    ...
    chem(i,k,j,P_nterpooh) = MAX ( REAL (oconv * var(ind_NTERPOOH), KIND=sp), 0.)  
    ...
    chem(i,k,j,P_phenol) = MAX ( REAL (oconv * var(ind_PHENOL), KIND=sp), 0.)  
    ...
    chem(i,k,j,P_terpnit) = MAX ( REAL (oconv * var(ind_TERPNIT), KIND=sp), 0.)  
    ...
    chem(i,k,j,P_xylenes) = MAX ( REAL (oconv * var(ind_XYLENES), KIND=sp), 0.)  
    ...
    chem(i,k,j,P_isopao2) = MAX ( REAL (oconv * var(ind_ISOPAO2), KIND=sp), 0.)  
    chem(i,k,j,P_isopbo2) = MAX ( REAL (oconv * var(ind_ISOPBO2), KIND=sp), 0.)  
...
END SUBROUTINE  t1_mozcart_interface


END MODULE module_kpp_t1_mozcart_interf 
