MODULE module_aerosols_sorgam_vbs
...
CONTAINS
   ...
   SUBROUTINE  sorgam_vbs_driver ( id,ktau,dtstep,t_phy,moist,aerwrf,p8w,  &
...
END SUBROUTINE sorgam_vbs_driver


SUBROUTINE sum_pm_sorgam_vbs (                                         &
   ...
   INTEGER :: i,ii,j,jj,k,n
      ...
      tsoa(its:ite, kts:kte, jts:jte)    = 0.
      asoa(its:ite, kts:kte, jts:jte)    = 0.
      ...
      do j=jts,jte
         ...
         enddo
      enddo
    END SUBROUTINE sum_pm_sorgam_vbs


SUBROUTINE     sorgam_vbs_depdriver (id,config_flags,ktau,dtstep,                        &
...
INTEGER :: i,j,k,l
      ...
      cblk(1,VCOR3    ) =   cor3(i,k,j)
        ...
        ddflx(i,j,p_asoa1j)=ddflx(i,j,p_asoa1j)+chem(i,k,j,p_asoa1j)/alt(i,k,j)*VGSA(i,j,VASOA1J)*dtstep
        ddflx(i,j,p_asoa1i)=ddflx(i,j,p_asoa1i)+chem(i,k,j,p_asoa1i)/alt(i,k,j)*VGSA(i,j,VASOA1I)*dtstep
        ddflx(i,j,p_asoa2j)=ddflx(i,j,p_asoa2j)+chem(i,k,j,p_asoa2j)/alt(i,k,j)*VGSA(i,j,VASOA2J)*dtstep
        ddflx(i,j,p_asoa2i)=ddflx(i,j,p_asoa2i)+chem(i,k,j,p_asoa2i)/alt(i,k,j)*VGSA(i,j,VASOA2I)*dtstep
        ddflx(i,j,p_asoa3j)=ddflx(i,j,p_asoa3j)+chem(i,k,j,p_asoa3j)/alt(i,k,j)*VGSA(i,j,VASOA3J)*dtstep
        ddflx(i,j,p_asoa3i)=ddflx(i,j,p_asoa3i)+chem(i,k,j,p_asoa3i)/alt(i,k,j)*VGSA(i,j,VASOA3I)*dtstep
        ddflx(i,j,p_asoa4j)=ddflx(i,j,p_asoa4j)+chem(i,k,j,p_asoa4j)/alt(i,k,j)*VGSA(i,j,VASOA4J)*dtstep
        ddflx(i,j,p_asoa4i)=ddflx(i,j,p_asoa4i)+chem(i,k,j,p_asoa4i)/alt(i,k,j)*VGSA(i,j,VASOA4I)*dtstep
        ddflx(i,j,p_bsoa1j)=ddflx(i,j,p_bsoa1j)+chem(i,k,j,p_bsoa1j)/alt(i,k,j)*VGSA(i,j,VBSOA1J)*dtstep
        ddflx(i,j,p_bsoa1i)=ddflx(i,j,p_bsoa1i)+chem(i,k,j,p_bsoa1i)/alt(i,k,j)*VGSA(i,j,VBSOA1I)*dtstep
        ddflx(i,j,p_bsoa2j)=ddflx(i,j,p_bsoa2j)+chem(i,k,j,p_bsoa2j)/alt(i,k,j)*VGSA(i,j,VBSOA2J)*dtstep
        ddflx(i,j,p_bsoa2i)=ddflx(i,j,p_bsoa2i)+chem(i,k,j,p_bsoa2i)/alt(i,k,j)*VGSA(i,j,VBSOA2I)*dtstep
        ddflx(i,j,p_bsoa3j)=ddflx(i,j,p_bsoa3j)+chem(i,k,j,p_bsoa3j)/alt(i,k,j)*VGSA(i,j,VBSOA3J)*dtstep
        ddflx(i,j,p_bsoa3i)=ddflx(i,j,p_bsoa3i)+chem(i,k,j,p_bsoa3i)/alt(i,k,j)*VGSA(i,j,VBSOA3I)*dtstep
        ddflx(i,j,p_bsoa4j)=ddflx(i,j,p_bsoa4j)+chem(i,k,j,p_bsoa4j)/alt(i,k,j)*VGSA(i,j,VBSOA4J)*dtstep
        ddflx(i,j,p_bsoa4i)=ddflx(i,j,p_bsoa4i)+chem(i,k,j,p_bsoa4i)/alt(i,k,j)*VGSA(i,j,VBSOA4I)*dtstep
...
END SUBROUTINE sorgam_vbs_depdriver
  ...
  DATA beta0(2,1)/4.6465E-2/, beta1(2,1)/ -0.54196/,cgama(2,1)/ -1.2683E-3/ 
  DATA beta0(2,2)/ -7.26224E-3/, beta1(2,2)/ -1.168858/,cgama(2,2)/3.51217E-5/ 
...
SUBROUTINE aeroproc(blksize,nspcsda,numcells,layer,cblk,dt,blkta,blkprs, &
    ...
    SUBROUTINE aerostep(layer,blksize,nspcsda,numcells,cblk,dt,so4rat         &
      ...
      REAL getaf2
      REAL aaa, xnum, xm3, fnum, fm3, phnum, phm3 
      REAL erf, & 
        erfc
...
REAL orgbio4rat(blksize)
      ...
      CALL sorgam_vbs(layer,blkta,blkprs,organt1rat,organt2rat,organt3rat, &
...
END SUBROUTINE rpmares_old
    ...
    SUBROUTINE rpmmod3(nspcsda,blksize,layer,dtsec,pres,temp,relhum, &
      ...
      blkprs(blksize)= pres*100. 

      blkrh(blksize) = relhum 
...
SUBROUTINE sorgam_vbs(layer,blkta,blkprs,organt1rat,organt2rat,organt3rat,  &
    organt4rat,orgbio1rat,orgbio2rat,orgbio3rat,orgbio4rat,drog,ldrog_vbs,ncv, &
    nacv,cblk,blksize,nspcsda,numcells,dt,igrid,jgrid,kgrid,brrto)
      ...
      REAL alphlowN(nbin,nsaprc) 
      REAL alphhiN(nbin,nsaprc)  
      REAL alphai(nbin,nsaprc)   
      
      REAL mwvoc(nsaprc)         

      REAL PnGtotal,DUM,FMTOT,FMTOT2,DUM2    
      integer, save :: icall
...
loop_cells: DO lcell = 1, numcells  
                DO l= 1, ldrog_vbs-1
                   drog(lcell,l) = f(l)*drog(lcell,l)
                END DO

                
                brrto= drog(lcell,pbrch) 
                DO bn=1,nbin  
                   DO cls=1,nsaprc 
...
do ll=1,nbin
 ...
 minit=  cblk(lcell,vorgpaj) + cblk(lcell,vorgpai) 
...
loop_newt:  DO LL=1,5   
               ...
               MTOT=AMAX1(MTOT,MINIT)  
               MTOT=AMIN1(MTOT,PnGtotal+minit)  
END DO  loop_newt   
...
SUBROUTINE         aerosols_sorgam_vbs_init(chem,convfac,z_at_w,                   &
   ...
   TYPE (grid_config_rec_type) , INTENT (in) :: config_flags
     ...
     real tempfac,mwso4,zz
      ...
      DATA so4vaptoaer/.999/
        ...
        call aerosols_sorgam_vbs_init_aercld_ptrs( &
        ...
        tsoa(its:ite, kts:kte-1, jts:jte)    = 0.
        asoa(its:ite, kts:kte-1, jts:jte)    = 0.
        bsoa(its:ite, kts:kte-1, jts:jte)    = 0.
        ...
        elseif( aer_ic_opt == AER_IC_PNNL ) then
           ...
           call sorgam_vbs_init_aer_ic_pnnl(   &
                chem, zz, i,k,j, ims,ime,jms,jme,kms,kme )
        else
           call wrf_error_fatal3("<stdin>",6602,&
                "aerosols_sorgam_vbs_init: unable to parse aer_ic_opt" )
        end if
    ...
    return
    END SUBROUTINE aerosols_sorgam_vbs_init

    SUBROUTINE aerosols_sorgam_vbs_init_aercld_ptrs(   &
         num_chem, is_aerosol, config_flags )
    ...
    END SUBROUTINE aerosols_sorgam_vbs_init_aercld_ptrs
    ...
    SUBROUTINE sorgam_vbs_init_aer_ic_pnnl(                  &
      ...
      chem(i,k,j,p_asoa1j) = conmin
      chem(i,k,j,p_asoa1i) = conmin
      chem(i,k,j,p_asoa2j) = conmin
      chem(i,k,j,p_asoa2i) = conmin
      chem(i,k,j,p_asoa3j) = conmin
      chem(i,k,j,p_asoa3i) = conmin
      chem(i,k,j,p_asoa4j) = conmin
      chem(i,k,j,p_asoa4i) = conmin
      chem(i,k,j,p_bsoa1j)  = conmin
      chem(i,k,j,p_bsoa1i)  = conmin
      chem(i,k,j,p_bsoa2j)  = conmin
      chem(i,k,j,p_bsoa2i)  = conmin
      chem(i,k,j,p_bsoa3j)  = conmin
      chem(i,k,j,p_bsoa3i)  = conmin
      chem(i,k,j,p_bsoa4j)  = conmin
      chem(i,k,j,p_bsoa4i)  = conmin
      chem(i,k,j,p_seas)     = mult*1.75


    END SUBROUTINE sorgam_vbs_init_aer_ic_pnnl

SUBROUTINE sorgam_vbs_addemiss( id, dtstep, u10, v10, alt, dz8w, xland, chem,                      &
                             ebu,                                                               &
  ...
  USE module_state_description, only:  num_chem,num_emis_seas2
  ...
  REAL, DIMENSION( ims:ime, kms:kme, jms:jme, num_chem ),               &
   ...
   REAL, DIMENSION( ims:ime, 1, jms:jme,num_emis_seas2),OPTIONAL,           &
         INTENT(INOUT ) ::                                                 &
         emis_seas2
  ...
  chem(its:ite,kts:kemit,jts:jte,p_nu0) =                        &
       ...
       anthfac*emis_ant(its:ite,kts:kemit,jts:jte,p_e_eci)   +   &
       nafac*emis_ant(its:ite,kts:kemit,jts:jte,p_e_naai)    +   &
  ...
  chem(its:ite,kts:kemit,jts:jte,p_ac0) =                        &
       ...
       anthfac*emis_ant(its:ite,kts:kemit,jts:jte,p_e_ecj)   +   &
       nafac*emis_ant(its:ite,kts:kemit,jts:jte,p_e_naaj)    +   &
  ...
  chem(its:ite,kts:kemit,jts:jte,p_corn) =                       &
       ...
       emis_ant(its:ite,kts:kemit,jts:jte,p_e_pm10) 
  ...
  chem(its:ite,kts:kemit,jts:jte,p_antha) =                      &
       chem(its:ite,kts:kemit,jts:jte,p_antha) +                 &
       emis_ant(its:ite,kts:kemit,jts:jte,p_e_pm10)*factor(its:ite,kts:kemit,jts:jte) 
  ...
  if( dust_opt == 5 ) then
      call sorgam_vbs_dust_gocartemis(                                &
...
END SUBROUTINE sorgam_vbs_addemiss


SUBROUTINE sorgam_vbs_seasalt_emiss(                                        &
     dtstep, u10, v10, alt, dz8w, xland, chem,                          &
   ...
   integer :: i, j, k, l, l_na, l_cl, n
    ...
    do i = its, ite
       ...
       emis_seas2(i,1,j,p_eseasj)=ssemfact_mass_j*dumspd10/1.0e6
       emis_seas2(i,1,j,p_eseasc)=ssemfact_mass_c*dumspd10/1.0e6


    end do 
    end do 
END SUBROUTINE sorgam_vbs_seasalt_emiss


   subroutine sorgam_vbs_dust_emiss(  slai,ust, smois, ivgtyp, isltyp,         &
...
1830    continue
   ...
   END subroutine sorgam_vbs_dust_emiss




  subroutine sorgam_vbs_dust_gocartemis (ktau,dt,num_soil_layers,alt,u_phy,    &
         v_phy,chem,rho_phy,dz8w,smois,u10,v10,p8w,erod,                   &
         ivgtyp,isltyp,xland,dx,g,                                         &
         ids,ide, jds,jde, kds,kde,                                        &
         ims,ime, jms,jme, kms,kme,                                        &
  ...
  do i=its,ite
    ...
    call sorgam_vbs_source_du( nmx, dt,i,j, &
...
end subroutine sorgam_vbs_dust_gocartemis

  SUBROUTINE sorgam_vbs_source_du( nmx, dt1,i,j, &
...
END SUBROUTINE sorgam_vbs_source_du



   subroutine wetscav_sorgam_vbs_driver (id,ktau,dtstep,ktauc,config_flags,      &
   ...
   end subroutine wetscav_sorgam_vbs_driver




END Module module_aerosols_sorgam_vbs
