...
MODULE module_kpp_mozart_mosaic_4bin_interf 
  ...
  USE mozart_mosaic_4bin_UpdateRconstWRF
  USE mozart_mosaic_4bin_Integrator
...
SUBROUTINE  mozart_mosaic_4bin_interface( &
  ...
  REAL(kind=dp) :: es, qvs, rh
   ...
   RCNTRL(3) = 0.01_dp * TIME_END
    ...
    var(ind_VOCA) = conv  * REAL( MAX(chem(i,k,j,P_voca),0.), KIND=dp)  
    var(ind_VOCBB) = conv  * REAL( MAX(chem(i,k,j,P_vocbb),0.), KIND=dp)  
    var(ind_SMPA) = conv  * REAL( MAX(chem(i,k,j,P_smpa),0.), KIND=dp)  
    var(ind_SMPBB) = conv  * REAL( MAX(chem(i,k,j,P_smpbb),0.), KIND=dp)  
  ...
  rh = MIN ( MAX ( rh, 0._dp), 1._dp)
   ...
   CALL mozart_mosaic_4bin_Update_Rconst(  &
  ...
  CALL mozart_mosaic_4bin_INTEGRATE(TIME_START, TIME_END, &  
   ...
   ENDIF
    ...
    chem(i,k,j,P_voca) = MAX ( REAL (oconv * var(ind_VOCA), KIND=sp), 0.)  
    chem(i,k,j,P_vocbb) = MAX ( REAL (oconv * var(ind_VOCBB), KIND=sp), 0.)  
    chem(i,k,j,P_smpa) = MAX ( REAL (oconv * var(ind_SMPA), KIND=sp), 0.)  
    chem(i,k,j,P_smpbb) = MAX ( REAL (oconv * var(ind_SMPBB), KIND=sp), 0.)  
...
END SUBROUTINE  mozart_mosaic_4bin_interface


END MODULE module_kpp_mozart_mosaic_4bin_interf 
