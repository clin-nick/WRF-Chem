...
TimeLoop: DO WHILE ( (Direction > 0).AND.((T-Tend)+Roundoff <= ZERO) &
   ...
   IF( T == Tstart ) THEN
     CALL radm2_IRRFun( Y, FIX, RCONST, IRR_WRK )
   ENDIF
...
SUBROUTINE radm2_IRRFun ( V, F, RCT, IRR )
...
10      Eps = Eps*2
   ...
   SUBROUTINE decomp_radm2( JVS, IER )
